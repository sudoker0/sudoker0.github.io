...
document.querySelectorAll("h3#accbar a").forEach(function (v) { document.querySelector("h3#accbar_small").append(v.cloneNode(true)); });
...
function check() {
    if (Number(gPBName("sv_cheat")) >= 1 || Number(gPBName("debug")) >= 1 || Number(gPBName("hack")) >= 1 || ls_gt("ls") == "927") {
    ...
    if (gPBName("id") == "rotate") {
        document.body.style.animation = "rotate_ 5s ease";
    }
    if (gPBName("id") == "rotate-inf") {
        document.body.style.animation = "rotate_ 5s ease infinite";
    }
}
document.addEventListener("click", function (evt) {
    var fE = sel("#accbar"), tE = evt.target;
    do {
        if (tE == fE) {
            return;
        }
...
window.onload = function () {
    smtg();
    setTimeout(function () { selAll("nav.navbar").forEach(function (e) { e.classList.add("logo_rotate"); }); }, Math.random() * (4.5e+6 - 3.6e+6) + 3.6e+6);
    setTimeout(function () { sel("#loadingIndicator").classList.add("lIClosed"); }, 500);
...
document.addEventListener("DOMContentLoaded", function () {
    ...
    document.querySelectorAll('.nt').forEach(function (v) {
...
document.onscroll = function (_) {
    dropShadow();
    sel("#accb_small").style["webkitClipPath"] = nBP.close;
    sel("#accb_small").style.clipPath = nBP.close;
    accb_small_isOn = false;
};
sel("#list_dir").onclick = function () { window.location.href = "/dir_listing.html"; };
var logo_a = selAll("nav#accb > a");
logo_a.forEach(function (e) {
    e.onmousemove = function (ev) {
        selAll("nav.navbar").forEach(function (e) {
            if (ev.ctrlKey && ev.shiftKey) {
                e.classList.add("logo_rotate");
            }
            else {
                e.classList.remove("logo_rotate");
            }
        });
    };
});
