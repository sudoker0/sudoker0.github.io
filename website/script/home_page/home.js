...
qSel("#claim_gift").onclick = () => __awaiter(this, void 0, void 0, function* () {
    if (claiming_gift)
        return;
    if (fartmode) {
        fartnoise.loop = true;
    ...
    claiming_gift = true;
    qSel("#claim_gift").style.transformOrigin = "bottom";
    qSel("#claim_gift").style.animation = "gift_opening_animation 3s";
    yield wait(2750);
    if (fartmode) {
        fartnoise.pause();
    ...
    qSel("#gift_display").style.display = "flex";
    qSel("#gift_flashbang").style.opacity = "1";
    yield wait(250);
    qSel("#gift_flashbang").style.transition = "opacity 0.5s";
    qSel("#gift_flashbang").style.opacity = "0";
    var gift = getRandomInt(1, 4);
    qSel(`#gift_item_${gift}`).style.display = "flex";
    yield wait(500);
    qSel("#claim_gift").style.animation = "none";
    yield wait(1500);
    var gift_svg = qSel(`div#gift_item_${gift} img`);
    var somestring = qSel("div#gift_thatsmygift_dialog p");
    qSel("#gift_thatsmygift_dialog").style.display = "block";
    switch (gift) {
        case 1:
    ...
    qSel("#gift_thatsmygift_dialog").style.display = "none";
    qSel(`#gift_item_${gift}`).style.display = "none";
    qSel("#gift_display").style.display = "none";
    qSel("#gift_flashbang").style.transition = "none";
    claiming_gift = false;
});
qSel("#website_logo").ondragstart = (ev) => {
    ev.dataTransfer.setData("text", "website_logo");
};
qSel("#website_logo").ondrag = (ev) => {
    console.log("Drag: " + ev);
};
qSel("#claim_gift").ondrop = (ev) => {
    ev.preventDefault();
    var data = ev.dataTransfer.getData("text");
    if (data != "website_logo")
        return;
    qSel("#claim_gift").style.animation = "lsd 1s infinite linear";
    fartmode = true;
};
qSel("#building_block").ondrop = (ev) => {
    ev.preventDefault();
    var data = ev.dataTransfer.getData("text");
    if (data != "website_logo")
        return;
    qSel("#building_block").setAttribute("src", qSel("#building_block").classList.contains("toggled")
        ? "./website/image/home_page/building_block_of_internet.svg"
        : "./website/image/home_page/building_block_of_my_website.svg");
    qSel("#building_block_caption").innerText =
        qSel("#building_block").classList.contains("toggled")
            ? "The Building Block Of The Internet"
            : "The Building Block Of My Website";
    qSel("#building_block").classList.toggle("toggled");
};
qSel("#claim_gift").ondragover = (ev) => ev.preventDefault();
qSel("#building_block").ondragover = (ev) => ev.preventDefault();
