...
HTMLElement.prototype.replace = function (
    ...
    for (const i in data) {
        ...
        const span: () => HTMLElement | null = () =>
            _this().querySelector(
                `span.reactive#${alternate_prefix}${encodeURIComponent(i)}`,
            );
        if (span() == null)
 ...
 */
function qSelAll<T extends Node>(sel: string) {
    return document.querySelectorAll(sel) as unknown as NodeListOf<T>;
}
...
function rString(l: number): string {
    ...
    for (var i = 0; i < l; i++) {
        r.push(
            String.fromCharCode.apply(null, [Math.floor(Math.random() * 256)]),
        );
    }
    return r.join("");
}
...
function updateWebsiteData() {
    ...
    qSel<HTMLElement>(".footer").replace({
        day: displayTime(
            new Date().getTime() - new Date(WEBSITE_CREATION_DATE).getTime(),
        ),
    });
...
function registerCardHover() {
    qSelAll<HTMLElement>(".card").forEach((v) => {
...
document.addEventListener("DOMContentLoaded", () => {
    ...
    switch (gPBName("id")) {
        case "entropy":
            var charStart = 1;
            setInterval(() => {
                var lChar = Math.round(
                        Math.random() * document.body.innerHTML.length,
                    ),
    ...
    qSelAll<HTMLElement>("section").forEach((v) => {
        if (intersect == null) {
            toggleSection(v.id);
            return;
        }
